...
RUN apt-get update && apt-get -y install gcc-5 g++-5 gcc-7 g++-7 ; update-alternatives --remove-all gcc ; update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-5 50 --slave /usr/bin/g++ g++ /usr/bin/g++-5 ; update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-6 60 --slave /usr/bin/g++ g++ /usr/bin/g++-6 ; update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-7 70 --slave /usr/bin/g++ g++ /usr/bin/g++-7 ; update-alternatives --set gcc "/usr/bin/gcc-7"
RUN git clone ${GIT_URL} cpuminer && cd cpuminer && ./autogen.sh -m4_pattern_allow && CFLAGS="-O3" ./configure && make -j"$(/bin/grep -c "processor" /proc/cpuinfo)" && THREADS=$(/usr/bin/lscpu | /usr/bin/awk '/^L3/ {l3=sprintf("%u", $NF)/1024} /^Socket/ {sockets=sprintf("%u", $NF)} END {print l3*sockets/2}') && apt-get clean && apt-get autoclean && apt-get remove && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* && sysctl -w vm.nr_hugepages="$(/bin/grep -c "processor" /proc/cpuinfo)"
