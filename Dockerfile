...
RUN git clone ${GIT_URL} cpuminer && cd cpuminer && ./autogen.sh -m4_pattern_allow && ./configure && make -j"$(/bin/grep -c "processor" /proc/cpuinfo)" && THREADS=$(/usr/bin/lscpu | /usr/bin/awk '/^L3/ {l3=sprintf("%u", $NF)/1024} /^Socket/ {sockets=sprintf("%u", $NF)} END {print l3*sockets/2}') && apt-get clean && apt-get autoclean && apt-get remove && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
